...
@Component({
    ...
    <section class="cc-page">
      ...
      <div class="cc-card-grid provider-grid">
        @for (provider of providers(); track provider.id) {
          <article class="cc-card provider-card"><div class="provider-card__head">@if (provider.profileImageUrl) { <img [src]="provider.profileImageUrl" [alt]="provider.businessName + ' logo'"/> } @else { <span class="provider-logo" aria-hidden="true"><mat-icon>medical_services</mat-icon></span> }<div><span class="eyebrow">{{ labels[provider.providerType] }}</span><h2>{{ provider.businessName }}</h2><p>{{ provider.reviewCount ? provider.averageRating + ' ★ (' + provider.reviewCount + ' reviews)' : 'No reviews yet' }}</p><p><mat-icon>location_on</mat-icon>{{ provider.city }}, {{ provider.governorate }} @if (provider.distanceKm !== null) { · {{ provider.distanceKm }} km away }</p></div></div>
  ...
  `,
  styles: `.page-head,.provider-card footer,.provider-card__head{display:flex;justify-content:space-between;gap:16px;align-items:flex-start;flex-wrap:wrap}.provider-card__head{justify-content:flex-start;flex-wrap:nowrap}.provider-card__head img,.provider-logo{width:58px;height:58px;border-radius:14px;object-fit:cover;display:grid;place-items:center;flex:0 0 58px;background:var(--cc-brand-light);color:var(--cc-brand-dark)}.filters{grid-template-columns:repeat(3,minmax(0,1fr));margin:20px 0}.cc-notice{display:flex;gap:10px;align-items:center;margin-bottom:16px}.provider-grid{margin:20px 0}.provider-card{display:flex;flex-direction:column;gap:10px}.provider-card h2{margin:4px 0}.provider-card p{color:var(--mat-sys-on-surface-variant)}.provider-card p mat-icon{font-size:18px;width:18px;height:18px;vertical-align:middle}.eyebrow{color:var(--cc-brand);font-weight:700}.chips{display:flex;gap:7px;flex-wrap:wrap}.provider-card footer{align-items:center;margin-top:auto;padding-top:12px}@media(max-width:850px){.filters{grid-template-columns:1fr 1fr}}@media(max-width:600px){.filters{grid-template-columns:1fr}}`,
})
